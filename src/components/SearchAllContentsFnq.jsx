...
import './SearchAllContentsFnq.css';
import { useState } from 'react';
import { useNavigate } from 'react-router';
import { apiCall } from '../apiService/apiService';

export default function SearchAllContentsFnq({ noticeSearch }) {

    if (noticeSearch) {
        noticeSearch = noticeSearch.filter(item => (item.nottype === 'B'));
    }
    ...
    const noticeDetail = (e) => {
        ...
        apiCall(url, 'get', null, null)
            .then((response) => {
                navigate("/FaqControllPageDetailPage?notnum=" + e, { state: response })
            }).catch((error) => {
        ...
        <div className='SearchAllContents_Containor'>
            {noticeSearch && noticeSearch.length > 0 ? noticeSearch.slice(indexOfFirstItem, indexOfLastItem).map((it, index) => (
                <div className='SearchAllContents_Box'>
                    ...
                    <div className='SearchAllContents_ContentsLocation'>자주하는질문</div>
                </div>
            ))
                :
